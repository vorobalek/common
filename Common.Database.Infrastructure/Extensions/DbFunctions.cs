...
namespace Common.Database.Infrastructure.Extensions
{
    public static class DbFunctions
    {
        ...
        {
            if (ExecutionEnvironment.IsInMemoryDatabase && alternateFunc != null)
            {
                return alternateFunc(input);
            }
        ...
        public static int? DatePart(string datePartArg, DateTime? date) 
            => Assert<DateTime?, int?>(nameof(DatePart), null, default);
        public static int? DatePart(string datePartArg, DateTime date) 
            => Assert<DateTime, int?>(nameof(DatePart), null, default);
        
        public static int? DatePart(string datePartArg, DateTimeOffset? date) 
            => Assert<DateTimeOffset?, int?>(nameof(DatePart), null, default);
        ...
        public static DayOfWeek? DayOfWeekDb(this DateTime date)
            => Assert<DateTime, DayOfWeek?>(nameof(DayOfWeekDb), e => e.DayOfWeek, date);
        ...
        /// <exception cref="InvalidOperationException"></exception>
        public static DayOfWeek? DayOfWeekDb(this DateTime? date)
            => Assert(nameof(DayOfWeekDb), e => e?.DayOfWeek, date);
        ...
        public static DayOfWeek? DayOfWeekDb(this DateTimeOffset date)
            => Assert<DateTimeOffset, DayOfWeek?>(nameof(DayOfWeekDb), e => e.DayOfWeek, date);
            ...
            where TContext : DbContext, ICommonDbContext<TContext>
        {
            foreach (
                var methodInfo in new[]
                {
                    ...
                    typeof(DbFunctions).GetRuntimeMethod(nameof(DatePart), new[] { typeof(string), typeof(DateTime) })!,
                    typeof(DbFunctions).GetRuntimeMethod(nameof(DatePart), new[] { typeof(string), typeof(DateTimeOffset?) })!,
                    typeof(DbFunctions).GetRuntimeMethod(nameof(DatePart), new[] { typeof(string), typeof(DateTimeOffset) })!
                })
            {
                modelBuilder
