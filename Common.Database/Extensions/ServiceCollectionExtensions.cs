...
namespace Common.Database.Extensions
{
    public static class ServiceCollectionExtensions
    {
            ...
            params Assembly[] assemblies)
        {
            ...
            {
                var type = listener.GetGenericTypeArgumentsOfSpecificParent(typeof(EntityChangeListener<>)).FirstOrDefault();
            ...
            params Assembly[] assemblies)
        {
            ...
            var listeners = types
                ...
                .Distinct()
                .Select(type => new {Type = type, Args = type.GetGenericTypeArgumentsOfSpecificParent(typeof(EntityTraitChangeListener<,>))})
            ...
            foreach (var listener in listeners)
            {
                ...
                if (!listener.Type.ContainsGenericParameters || 
                    ...
                    !listener.Type.GetGenericArguments()[0].GetGenericParameterConstraints()[0].HasInterface(typeof(IEntityTrait)))
                {
                    throw new ArgumentException($"The {listener.Type.FullName} must be a generic type and have a single generic parameter with type cast to {typeof(IEntityTrait).FullName}");
                }
            ...
            params Assembly[] assemblies)
        {
            ...
            var listeners = types
                ...
                .Distinct()
                .Select(type => new {Type = type, Args = type.GetGenericTypeArgumentsOfSpecificParent(typeof(EntityHostChangeListener<,>))})
                .Where(t => t.Args.Length == 2);
            ...
            foreach (var listener in listeners)
            {
                ...
                foreach (var type in hostTypes)
                {
                    if (type.GetInterface(host.Name) == null ||
                        ...
                        throw new ArgumentException($"The {type.FullName} can not contains interfaces with undefined generic arguments");
                    
                    if (!listener.Type.ContainsGenericParameters ||
                        listener.Type.GetGenericArguments().Count(a => a.IsGenericParameter) != type.GetInterface(host.Name)?.GetGenericArguments().Length) 
                        throw new ArgumentException($"The {listener.Type.Name} must be a generic type and have as many generic parameters as the interface {host.Name}");
                    
                    services.AddEntityChangeListener(type, listener.Type.MakeGenericType(type.GetInterface(host.Name)!.GetGenericArguments()));
                }
            }
                ...
                throw new ApplicationException(
                    ...
                    string.Join(
                        ...
                        types.Select(t => $"{t.Name} is inheritance from " +
                                          string.Join(" ", t.GetInterfaces().Where(i => i != typeof(IEntityTrait) && i != typeof(IEntity))))));
        }
            ...
            if (types.Length > 0)
                throw new ApplicationException(
                    ...
                    string.Join(
                        Environment.NewLine, 
                        types.Select(t => $"{t.Name} is inheritance from " +
                                          string.Join(" ", t.GetInterfaces().Where(i => i != typeof(IEntityHost<>) && i != typeof(IEntity))))));
        }
    }
}